...
from ftl.node import node_builder
...
parser.add_argument('--base', action='store', required=True,
...
def main():
    ...
    with context.Workspace(args.directory) as ctx:
        with cache.Registry(
            ...
            with node_builder.NodeApp(ctx) as bldr:
                with docker_image.FromRegistry(
                  base_name, base_creds, transport) as base_image:
                    ...
                    with bldr.CreatePackageBase(base_image, cash) as bwd:
                        # Construct the application layer from the context.
                        app_layer, _ = bldr.BuildAppLayer()
                        with append.Layer(bwd, app_layer) as app_image:
                            print('Uploading app layer.')
                            with docker_session.Push(
                                target_image, target_creds, transport,
                                    threads=_THREADS, mount=[base_name]) as s:
                                s.upload(app_image)
